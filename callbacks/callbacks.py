...
from BoundingCircles import BoundingCircles
        ...
        for bbox_gt, class_gt, clas_pred, bbox_pred in \
            ...
            for box, cla in zip(bbox_gt, class_gt):
                if (bbox_gt.shape[-1] == 4):
                    ...
                    temp = BoundingCircle(imageName=str(self.imageCounter), classId=self.metric_names_original[cla], x=box[0], y=box[1],
...
#            self.metrics += max(sum([mc['F1'] for mc in metricsPerClass]) / len(metricsPerClass), 0)
